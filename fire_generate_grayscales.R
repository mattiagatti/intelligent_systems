...
for (image_path in c(train_images, test_images)) {
  ...
  image <- resize(image, size_x = 600, size_y = 600)
    ...
    plot(image)
    grayscale_path <- sub("train", "train_gray", image_path)
    grayscale_path <- sub("test", "test_gray", grayscale_path)
    dir.create(dirname(grayscale_path), recursive = TRUE, showWarnings = FALSE)
    save.image(image, grayscale_path, quality = 1)
    }
  )
}